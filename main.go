...
// It deletes all completed (done=true) tasks and prints how many were cleared
// and how many tasks remain.
// Returns an error instead of calling os.Exit, enabling unit testing.
func runClear(mgr *task.Manager, args []string) error {
	fs := flag.NewFlagSet("clear", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	// clear accepts no flags; parse is still called to handle unexpected flags gracefully.
	if err := fs.Parse(args); err != nil {
	...
	// Determine how many tasks remain after the clear operation.

...
// On success prints "Task added: <id>" so the smoke test can extract the ID.
func runAdd(mgr *task.Manager, args []string, w io.Writer) error {
	...
	// Validate priority at the CLI boundary so invalid values return non-zero exit.
	switch *priority {
	case "high", "medium", "low":
		// valid
	default:
		return fmt.Errorf("add: invalid priority %q: must be high, medium, or low", *priority)
	}

	id, err := mgr.AddAndReturnID(title, *priority, *due)
	if err != nil {
		return fmt.Errorf("add: %w", err)
	}
	fmt.Fprintf(w, "Task added: %d\n", id)
	return nil
}
